...
local function load_manifest(path)
    ...
    C.close(fd)
    if ret ~= len then
        error(ret < 0 and posix.strerror(err) or string.format("short read, %u/%u", ret, len))
    end
...
local function save_manifest(path, manifest, compression_level)
    ...
    local fd = posix.create(path)
    posix.write(fd, ptr, len)
    C.free(ffi.gc(ptr, nil))
    C.close(fd)
end
...
function TarXz:open(filename, manifest)
    ...
    local fd = posix.open(filename)
    self.fd = fd
    local stream_buf = ffi.new("uint8_t[?]", xz.LZMA_STREAM_HEADER_SIZE)
    local ret
    -- Check header.
    posix.read(fd, stream_buf, xz.LZMA_STREAM_HEADER_SIZE)
    ...
    assert(check_size > 0)
    -- Check footer.
    posix.lseek(fd, -xz.LZMA_STREAM_HEADER_SIZE, C.SEEK_END)
    posix.read(fd, stream_buf, xz.LZMA_STREAM_HEADER_SIZE)
    ...
    posix.read(fd, index_buf, footer_stream_flags.backward_size)
...
function TarXz:rewrite(entries)
    ...
    assert(ret == xz.LZMA_OK, ret)
    posix.write(fd, stream_buf, C.LZMA_STREAM_HEADER_SIZE)
    -- Second: the blocks.
    local offset = C.LZMA_STREAM_HEADER_SIZE
    local by_path = {}
    local buf = buffer:new()
    for i, e in ipairs(entries) do
        posix.lseek(self.fd, e.xz_offset)
        e.xz_offset = offset
    ...
    assert(ret == xz.LZMA_OK, ret)
    posix.write(fd, index_buf, index_size)
    -- Finally: the footer.
...
function Updater:fetch_manifest()
    ...
    self.update_url = url.absolute(self.manifest_url, manifest.filename)
...
function Updater:prepare_update(progress_cb)
    ...
    if self.seed then
        if type(self.seed) == "table" then
            matches = function(e)
                local se = self.seed[e.path]
                return se and se.size == e.size and se.hash == e.hash
            end
        else
            assert(lfs.attributes(self.seed, "mode") == "directory")
            matches = function(e)
                return path_matches(self.seed.."/"..e.path, e.size, e.hash)
            end
        end
    end
    for i, e in ipairs(self.manifest.files) do
        ...
        if progress_cb and not progress_cb(i, #self.manifest.files) then
            -- Canceled.
            return
        end
    end
...
function Updater:download_update(progress_cb)
    ...
    local fd = posix.create(update_file..".part")
    self.fd = fd
    local stream_buf = ffi.new("uint8_t[?]", xz.LZMA_STREAM_HEADER_SIZE)
    local ret
    -- Write header.
    ...
    if ret ~= 0 then
        error("posix_fallocate: "..posix.strerror(ret))
    end
    ...
    if not self.dl:fetch(self.update_url, function(ptr, len)
        ...
        if progress_cb and not progress_cb(downloaded, self.download_size, entry_index, entry_path) then
            return false
        end
        return true
    end, ranges) then
    ...
    if ret ~= 0 then
        error("ftruncate: "..posix.strerror())
    end
    C.close(fd)
    self.fd = nil
